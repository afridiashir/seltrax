    ...
    return (
        <div className="overflow-x-auto">
            {selectedIds.length > 0 && (
                <div className="text-sm p-2 px-2 pl-5 bg-background flex items-center border-b justify-between">
                    <p>{selectedIds.length} {selectedIds.length === 1 ? "Customer" : "Customers"} selected</p>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="ml-auto" size={"icon-sm"}>
                        ...
                        <DropdownMenuContent align="end" className="text-sm">
                            ...
                            <DropdownMenuItem className="cursor-pointer w-full p-2 hover:bg-red-600 hover:text-white bg-red-100 rounded text-red-600 hover:bg-red-500/90 flex items-center gap-2" onClick={() => deleteSelectedCustomers(selectedIds)}>
                    ...
                    {table.getHeaderGroups().map((headerGroup) => (
                        <TableRow key={headerGroup.id} >
                            {headerGroup.headers.map((header) => {
                                return (
                                    <TableHead key={header.id} className=" text-foreground text-sm">
